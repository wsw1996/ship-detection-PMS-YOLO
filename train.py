...
if __name__ == '__main__':
    # 加载模型
    # model = YOLO(r'yolov8-AFPN-4-LSK-all-ECA.yaml')
    # model = YOLO(r'Afpn+LSK+all.yaml').load("yolov8n.pt")
    model = YOLO(r'/root/ultralytics-main/ultralytics/runs-Sea429/train-V8-DCN+GAM/trainexp-V8-DCN+GAM/weights/last.pt')
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'/root/ultralytics-main/ultralytics/cfg/datasets/Ses_ships.yaml',
        ...
        project='runs-Sea429/train-V8-DCN+GAM',  # (str, optional) 项目名称
        name='trainexp-V8-DCN+GAM',  # (str, optional) 实验名称，结果保存在'project/name'目录下
